...
### [2026-08-13] `coerenza()` confronta un flusso di cassa dichiarato con un "reale" che esclude gli investimenti, e sale strutturalmente nel tempo
**Cosa**: `spese/revolut.py: coerenza()` confronta `D.risparmio_totale()` (`dichiarato`, la somma di `risparmi_periodo.effettivo_risparmio` — soldi che l'utente ha dichiarato essere usciti dal conto personale) contro `rev["risparmi"]` (`reale`, solo il totale dei salvadanai/deposito Revolut). Non include `rev["investimenti"]`.
**Perché si rompe**: quando una parte dei soldi dichiarati come risparmiati finisce investita invece di restare liquida nel deposito, quella quota esce dal conto personale (quindi *è* dichiarata) ma non compare in `rev["risparmi"]` — genera uno scarto negativo permanente ("risulta risparmiato più di quanto ci sia") che non è un errore di registrazione ma un limite del confronto. Verificato sui dati reali dell'utente (screenshot 13/08/2026): dichiarato € 11.873,26, reale (solo risparmi) € 8.525,63, scarto € 3.347,63. Letto l'estratto conto consolidato Revolut reale (`consolidated-statement-v2_2024-09-10_2026-08-13...xlsx`): nel periodo tracciato risultano € 4.444,28 netti versati verso il conto investimenti — ordine di grandezza compatibile con lo scarto osservato, a sostegno dell'ipotesi che parte del "dichiarato" sia in realtà finita investita. **Non è però una correzione banale**: sommare `investimenti` (valore di mercato attuale, comprende le plusvalenze) al `reale` farebbe divergere il confronto nella direzione opposta nel tempo, perché il dichiarato è un flusso di cassa storico (costo) mentre gli investimenti si rivalutano — un guadagno di mercato genererebbe un falso scarto positivo permanente, non un errore da correggere.
**Impatto**: l'avviso di "coerenza" su questa pagina non è affidabile quando l'utente investe parte del risparmio dichiarato: rischia di essere ignorato come "falso allarme cronico" proprio nei casi in cui servirebbe davvero (un periodo dimenticato, un importo scritto male).
**Stato**: non corretto — richiede una decisione dell'utente su come modellare gli investimenti nel confronto (tracciare il costo storico separatamente dal valore di mercato, o accettare un margine più largo, o altro). Chiesto in chat il 2026-08-13.
