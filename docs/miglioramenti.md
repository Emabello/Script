...
**Impatto**: due. (1) Il saldo di `/saldi` e della home è sbagliato per una parte di ogni mese, di un importo pari al risparmio del periodo — proprio nei giorni in cui si guarda "quanto posso spendere". (2) Ogni riconciliazione contro un estratto che cade in quella finestra vede uno scarto fantasma: è successo oggi, con una previsione del saldo bancario al 26/02/2026 sbagliata di esattamente 737,00.
**Stato**: aperto. La strada pulita è la stessa già proposta nella voce sopra — registrare i bonifici verso Revolut come movimenti veri in `spese`, con la loro data, e togliere `− risparmio_totale()` da `saldo_conto()`. In alternativa minima, `risparmi_periodo` dovrebbe portare anche la data effettiva dell'uscita e `saldo_conto()` usare quella.
